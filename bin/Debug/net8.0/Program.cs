...
{
    static async Task Main(string[] args)     //метод, который является точкой входа в программу
    {
        ...
        await CheckCreatedPasswordAsync();                                                  //вызов метода
    }
    public static async Task CheckCreatedPasswordAsync()                                       //Создание метода, который ничего не возвращает
    {
        ...
        if (!File.Exists(filePath))                                                 //Проверка, создан ли файл
        {
            using (FileStream file = new FileStream(filePath, FileMode.Create))     //Создания файла с именем из filePath (если файл не создан по условию из if)
            {
                ...
                await JsonSerializer.SerializeAsync(file, hashPassword);                       //Запись закодированного пароля в файл в формате json
            }
            await CryptoKeysCreator.CreateKeysAsync();                                         //Вызов метода из класса CryptoKeysCreator для создания
        ...
        else
        {
            ...
            {
                string line = await JsonSerializer.DeserializeAsync<string>(reader);           //Объявление переменной, которая хранит десериализированную строку из созданного файла
                ...
                else                                                                    //если условие false
                {
                    ...
                    await CheckCreatedPasswordAsync();                                             //циклиться метод, пока пароль не будет подтвержден. Перезапуск метода
                }
            }
        }
    }
